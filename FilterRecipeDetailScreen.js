...
function FilterRecipeDetailScreen({navigation}) {
  ...
  const config = {
   ...
   const getUser = async () => {
	const recipeRef =  firestore().collection("recipes").doc(global.config.collection).collection(global.config.diet_res).doc(global.config.id);
...
export default FilterRecipeDetailScreen;
